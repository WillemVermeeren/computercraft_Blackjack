...
---------------------- checks if eeverything is ok ------------------------
assert(speaker~=nil, "no spealer connected =[")
...
while true do
    ...
    if goTo == nil or goTo == "idleScreen" then
        goTo = nil
        if not fs.exists("disk/id.lua") then -- manages idle screen
            local idleCardXPosition1 = math.random(1, 38)
            local idleCardYPosition1 = -22

            local idleCardXPosition2 = math.random(1, 38)
            local idleCardYPosition2 = -60

            local invalidCardPromptTimer = 0

            function showIdleMenu()
                while true do
                    idleCardYPosition1 = idleCardYPosition1 + 1
                    if idleCardYPosition1 > 52 then
                        idleCardXPosition1 = math.random(1, 38)
                        idleCardYPosition1 = -22
                    end     -- manages x and y position of card 1

                    idleCardYPosition2 = idleCardYPosition2 + 1
                    if idleCardYPosition2 > 52 then
                        idleCardXPosition2 = math.random(1, 38)
                        idleCardYPosition2 = -22
                    end     -- manages x and y position of card 2

                    term.setBackgroundColor(colors.green)
                    term.clear()    -- clears screen

                    paintutils.drawImage(cardIdle, idleCardXPosition2, idleCardYPosition2)
                    paintutils.drawImage(cardIdle, idleCardXPosition1, idleCardYPosition1) --draws cards

                    paintutils.drawImage(blackjackIdle, 5, 5) -- draws blackjack logo thingy

                    paintutils.drawLine(1, 52, 57, 52, colors.blue)    -- draws "press screen to play" on the bottom of the monitor
                    term.setBackgroundColor(colors.blue)
                    term.setTextColor(colors.white)
                    term.setCursorPos(19, 52)
                    term.write("-click screen to play-")

                    if invalidCardPromptTimer > 0 then -- chows prompt when ivalid cards is inserted
                        invalidCardPromptTimer = invalidCardPromptTimer -1
                        paintutils.drawBox(1, 23, 57, 27, colors.white)

                        paintutils.drawFilledBox(1, 24, 57, 26, colors.blue)
                        term.setTextColor(colors.white)
                        term.setCursorPos(16, 25)

                        term.write("-please insert wvIDcard-")
                    end
                    sleep(0.1) -- change for speed (I recomend not going under 0.1 as it can get glitchy)
                end
            end
            function waitForKeyPress() os.pullEvent("monitor_touch") end

            while true do
                parallel.waitForAny(waitForKeyPress, showIdleMenu)
                if fs.exists("disk/id.lua") then
                    ...
                    invalidCardPromptTimer = 20 -- sets how long the invalidCard pormpt should be displayed
                end
            end
        end
        local idFile = fs.open("disk/id.lua", "r")
        local currentwvID = idFile.readAll()
        idFile.close()
    end
    --------------------------- bet making----------------------------------------
    if goTo == nil or goTo == "betMaking" then
        ...
        balance = getBalance(currentwvID)
        if balance > 0 then
            ...
            function checkForCard()
                while fs.exists("disk/id.lua")do sleep(0.1) end
                goTo = "idleScreen"               
            end
            ---------- drawing and changing betting amount ------------------
            while goTo==nil do
                parallel.waitForAny(updateBettingAmount, checkForCard)                
            end
        else
            goTo = "idleScreen"
        end
        ...
        elseif dealersCardCount>playersCardCount then
            ...
            if playersCardCount == 21 then
                if #playerCards == 2 then
                    --natural
                    changeBalance(bettingAmount*naturalMultiplier)
                    paintutils.drawImage(youWinImage, 12, 18)
                    speaker.playSound("block.note_block.pling", 1, 10)

                else
                    --blackjack
                    changeBalance(bettingAmount*blackjackMultiplier)
                    paintutils.drawImage(youWinImage, 12, 18)
                    speaker.playSound("block.note_block.pling", 1, 10)
                end
            else
